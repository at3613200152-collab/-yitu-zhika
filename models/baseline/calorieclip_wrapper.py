...
CalorieCLIP 基线封装
=====================
使用OpenCLIP的CalorieCLIP模型作为基线方法进行对比。

CalorieCLIP利用CLIP的视觉-语言对齐能力，通过文本描述
（如"这道菜含有500卡路里"）来估计食物热量。

参考: Mini et al., "CalorieCLIP: Calorie Estimation from Food Images 
...
class CalorieCLIPWrapper(nn.Module):
    ...
    使用OpenCLIP的预训练视觉编码器提取食物图像特征，
    然后通过回归头预测卡路里。

    本封装提供两种模式:
    1. 零样本模式: 直接使用CLIP的视觉-文本对齐
    2. 微调模式: 冻结CLIP视觉编码器，只训练回归头

    Args:
        model_name: OpenCLIP模型名称 (默认 'ViT-B-32')
        pretrained: 预训练权重标识 (默认 'laion2b_s34b_b79k')
        freeze_encoder: 是否冻结视觉编码器 (默认True)
        num_regression_outputs: 回归输出维度 (默认2: 卡路里+重量)
    """
    ...
    def __init__(
        ...
        try:
            ...
            print("警告: open_clip未安装，CalorieCLIP功能不可用")
        ...
        # 分类头: CLIP特征 → 食物类别
        self.classifier = None  # 延迟初始化（需要知道num_classes）
    ...
    def forward(self, image: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            dict: {
                'features':  [B, visual_dim]  CLIP视觉特征
                'nutrition': [B, 2]           回归输出
    ...
    def zero_shot_classify(
        ...
        with torch.no_grad():
            ...
            # 计算余弦相似度
            logits = (image_features @ text_features.T) * 100.0  # CLIP的temperature scaling
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 初始化分类头
        model.init_classifier(num_classes=61)
        ...
        print(f"CalorieCLIP测试失败（可能未安装open_clip）: {e}")
        print("请运行: pip install open-clip-torch")

...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, 'results', 'phase2_eval')
...
def evaluate_model():
    ...
    # 1. Val dataset
    print("\n--- Loading val dataset ---")
    val_set = Nutrition5kDataset(root_dir=DATA_DIR, split='val')
    val_loader = DataLoader(val_set, batch_size=16, shuffle=False, num_workers=0)
    print(f"Val samples: {len(val_set)}")

    # 2. Generator
    print("\n--- Loading Phase1 generator ---")
    generator = UNetGenerator(in_channels=3, out_channels=1, base_channels=64).to(DEVICE)
    ...
    model = ResNetMultiTask(num_classes=61, input_channels=4, pretrained=False).to(DEVICE)
    model_ckpt = torch.load(MODEL_CKPT, map_location=DEVICE, weights_only=False)
    model_state = model_ckpt.get('model_state_dict', model_ckpt)
    model.load_state_dict(model_state)
    model.eval()
    epoch = model_ckpt.get('epoch', '?')
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            all_pred_cal.extend(preds[:, 0])
            all_pred_wt.extend(preds[:, 1])

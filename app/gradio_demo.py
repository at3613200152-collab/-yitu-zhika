...
def create_demo():
    ...
    def analyze_food(image):
        ...
        try:
            ...
            # 分类和营养 (pipeline已expm1还原到物理空间)
            category = f"食物类别: {result.get('category', '未知')}"
    ...
    # ========= 食谱生成 =========
    def generate_recipe(height, weight, age, gender, activity, goal):
        """生成周食谱 (基于用户信息计算TDEE后生成)"""
        try:
    ...
    # ========= 构建Gradio界面 =========
    with gr.Blocks(title="一图知卡 - 食物热量分析") as demo:
        gr.Markdown(
...
if __name__ == "__main__":
    ...
    demo.launch(
        ...
        server_port=7860,
        share=False,
        theme=gr.themes.Soft(),  # Gradio 6.0: theme参数移到launch()
    )

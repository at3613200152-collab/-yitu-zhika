...
class InferencePipeline:
    ...
    def _load_generator(self, ckpt_path: str):
        ...
        # 处理可能的module.前缀
        state = {k.replace('module.', '').replace('generator.', ''): v
    ...
    def predict(self, image: Image.Image) -> Dict[str, Any]:
        ...
        # 预处理
        rgb_tensor = self.transform(image).unsqueeze(0).to(self.device)  # [1, 3, 256, 256]
        ...
        if self.generator is not None:
            with torch.no_grad():
                nir_tensor = self.generator(rgb_simple)  # 用简单归一化的输入
        else:

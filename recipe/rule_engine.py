...
class RecipeRuleEngine:
    ...
    def _solve_single_meal(
        ...
        deviation = total_calories - calorie_target
        prob += deviation * deviation  # 最小化偏差平方
        ...
        for category, min_count in category_rules.items():
            ...
            if cat_foods:
                ...
                for name in cat_foods:
                    is_selected = pulp.LpVariable(f"sel_{name}", cat='Binary')
    ...
    def validate_recipe(self, recipe: List[Dict]) -> Dict[str, Any]:
        ...
        for day in recipe:
            day_total = {"calories": 0.0, "protein": 0.0, "carb": 0.0, "fat": 0.0}

...
class Nutrition5kDataset(Dataset):
    ...
    def __init__(
        ...
        # 所有split共享同一类别映射，避免同一类别在train/val/test中编号不同
        categories = sorted(set(item["category"] for item in self.metadata.values()))
    ...
    def _load_metadata(self, metadata_csv: Optional[str]) -> Dict[str, Dict[str, Any]]:
        ...
        if metadata_csv is None:
            verified_csv = os.path.join(self.root_dir, "dishes_verified.csv")
            if os.path.exists(verified_csv):
                metadata_csv = verified_csv
            categorized_csv = os.path.join(self.root_dir, "dishes_categorized.csv")
            if metadata_csv is None and os.path.exists(categorized_csv):
                metadata_csv = categorized_csv
            # 自动搜索CSV文件: 先查dish_metadata/子目录, 再查根目录
            metadata_dir = os.path.join(self.root_dir, "dish_metadata")
            if metadata_csv is None and os.path.isdir(metadata_dir):

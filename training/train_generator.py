...
def train_generator(config: Dict, resume: bool = False):
    ...
    run_name = datetime.now().strftime('generator_%Y%m%d_%H%M%S')
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            # Pix2Pix标准数值域: RGB/NIR统一缩放到[-1,1](U-Net输出为tanh)
            rgb = batch['rgb'].to(device) * 2.0 - 1.0
            nir_real = batch['nir'].to(device) * 2.0 - 1.0
        ...
        with torch.no_grad():
            for val_batch in val_loader:
                rgb_v = val_batch['rgb'].to(device) * 2.0 - 1.0
                nir_v = val_batch['nir'].to(device) * 2.0 - 1.0
                nir_pred = model.generator(rgb_v)
                # PSNR在[0,1]域计算(反tanh缩放)
                nir_pred01 = (nir_pred + 1.0) / 2.0
                nir_v01 = (nir_v + 1.0) / 2.0
                # 计算PSNR
                mse = torch.mean((nir_pred01 - nir_v01) ** 2)
                if mse > 0:
